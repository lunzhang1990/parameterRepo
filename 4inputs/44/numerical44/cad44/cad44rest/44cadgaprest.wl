...
candidates= {61,89,123,322,372,390};
bound=Length[candidates];
ret={};
bvars = {\[Delta][1],\[Delta][2]};
fvars = {\[Delta][3],\[Delta][4]};
Timing[For[i=1,i<=bound,i++,index = candidates[[i]];instance=TimeConstrained[usereduce[asymmetricorders[[index]]+1,polys,restrictions,bvars,fvars],timebound];Print[{index,instance}];If[instance ==False,AppendTo[ret,index],None];];];

bvars = {\[Delta][3],\[Delta][4]};
fvars = {\[Delta][1],\[Delta][2]};
Timing[For[i=1,i<=bound,i++,index=candidates[[i]];instance=TimeConstrained[usereduce[asymmetricorders[[index]]+1,polys,restrictions,bvars,fvars],timebound];Print[{index,instance}];If[instance ==False,AppendTo[ret,index],None];];];
nodret = DeleteDuplicates[ret];
Export["nodret3srest.json",nodret];
]
